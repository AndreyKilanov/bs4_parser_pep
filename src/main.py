...
from utils import get_response, find_tag, info_pep
...
def pep(session):
    ...
    tags_tr = find_tag(numerical_index, 'tbody').find_all('tr')

    for tr in tqdm(tags_tr):
        status_arr = find_tag(tr, 'abbr')['title']
        url_pep = urljoin(PEPS_URL, find_tag(tr, 'a')['href'])
    ...
    downloads_dir.mkdir(exist_ok=True)
    file_path = downloads_dir / 'peps.csv'
